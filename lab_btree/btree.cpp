...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if(first_larger_idx <= subroot->elements.size() - 1 && subroot->elements[first_larger_idx].key == key)
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    /* TODO Your code goes here! */
    DataPair temp = *mid_elem_itr;
    parent->children.insert(child_itr, new_right);
    parent->elements.insert(elem_itr, temp);
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if(subroot == NULL)
        return;
    if(subroot->elements.size() != 0 && first_larger_idx <= subroot->elements.size() - 1 && subroot->elements[first_larger_idx].key == pair.key)
        return;     
    if(subroot->is_leaf){
