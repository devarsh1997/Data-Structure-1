...
void CommonWords::init_common()
{
    ...
    for(unsigned long i = 0; i < file_word_maps.size();i++)
    {
        for(map<string, unsigned int>::iterator it = file_word_maps[i].begin(); it != file_word_maps[i].end();it++)
        {
            ...
            if(lookup == common.end()) common[it->first] = 1;
...
vector<string> CommonWords::get_common_words(unsigned int n) const
{
    ...
    for(auto & it : common){
        if(it.second == file_word_maps.size()){
           ...
           for(unsigned long i = 0; i < file_word_maps.size(); i++)
           {
               auto lookup = file_word_maps[i].find(it.first);
               if( lookup->second < n) isGreater = false;
           }
           if(isGreater == true) out.push_back(it.first);
        }
    }
