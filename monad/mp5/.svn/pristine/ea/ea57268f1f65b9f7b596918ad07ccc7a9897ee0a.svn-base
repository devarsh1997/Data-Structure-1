...
void Quadtree::pruneHelp(QuadtreeNode* & root, int tolerance)
{
    ...
    if(checkLeaf(root, tolerance, root->element) == true){
        ...
        Quadtree::pruneHelp(root->nwChild, tolerance);
        Quadtree::pruneHelp(root->neChild, tolerance);
        Quadtree::pruneHelp(root->swChild, tolerance);
        Quadtree::pruneHelp(root->seChild, tolerance);
    }
...
int Quadtree::idealPrune(int numLeaves) const
{
    ...
    while(l < r){
        ...
        if(pruneSize(tolerance) == numLeaves){
            while(pruneSize(tolerance) == numLeaves){
                tolerance--;
            }
            return tolerance + 1;
        }
