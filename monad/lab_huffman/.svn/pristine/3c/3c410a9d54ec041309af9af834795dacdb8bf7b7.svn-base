...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while(singleQueue.size() + mergeQueue.size() > 1)
     {
         ...
         Frequency f(temp1->freq.getFrequency() + temp2->freq.getFrequency());
         TreeNode* r = new TreeNode(f);
         r->left = temp1; 
         r->right = temp2;
         mergeQueue.push(r);
     }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.getNextBit() == true)
    {
        ...
        result = new TreeNode(f);
    }
    else
    {
        Frequency f(0);
        result = new TreeNode(f);
        result->left = readTree(bfile);
        result->right = readTree(bfile);
    }
        
    return result; // replaceme!
}
