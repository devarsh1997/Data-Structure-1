...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
        ...
        string s = ll.customer + ":" + ll.url;
        if(whenVisitedTable.keyExists(s) == true){
            if(whenVisitedTable[s] < ll.date){
                whenVisitedTable[s] = ll.date;
            }
        }
        else{
            whenVisitedTable[s] = ll.date;
...
bool LogfileParser::hasVisited(const string& customer, const string& url) const
{
    ...
    return whenVisitedTable.keyExists(temp);
}
                                  ...
                                  const string& url) const
{
     ...
     * @todo Implement this function.
     */
     if(hasVisited(customer, url) == false)
         return time_t();
     string temp = customer + ":" + url;
